...
const CreateBlogUser = () => {
    ...
    const handleCreateBlogByBalance = async () => {
        try {
            setLoading(true);
            const minus = await axiosInstance.post("/minus_balance", { amount: 10000 });
            if (minus.data.message === "Success") {
                const blogData2 = {
                ...
                const respone = await axiosInstance.post("/blog", blogData2);
                setIsModalOpen(false);
                toast.success(respone.data.message);
            } else {
                navigate("/profile");
                toast.info("Vui lòng nạp thêm tiền vào tài khoản để tạo blog");
            }
            navigate("/blog");
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        if ((title && content && image && tags) || imagePreview) {
            try {
                ...
                const res = await axiosInstance.post("/create-blog-by-user", blogData);
                if (res.data.message === "You do not have enough points to create a blog") {
                    setLoading(false);
                    toast.success(res.data.message);
                    setIsModalOpen(true);
                } else if (res.data.message === "User package not found") {
                    setLoading(false);
                    toast.success(res.data.message);
                    navigate("/package");
                } else {
    ...
    `,
            ...
            <Modal
                title="Bạn Không Đủ Điểm Để Tạo Blog"
                visible={isModalOpen}
                onOk={handleCreateBlogByBalance}
                onCancel={() => setIsModalOpen(false)}>
                <p>Bạn có muốn dùng tiền trong tài khoản để tạo blog</p>
            </Modal>
        </div>
    );
};
